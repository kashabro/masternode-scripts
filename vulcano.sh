...
COIN_TGZ='https://github.com/kashabro/masternode-scripts/files/2439812/vulcano-2.0.0.0-linux64.zip'
...
function download_node() {
    ...
    dir=`mktemp -d`
    if [ -z "$dir" ]; then
        # Create directory under $HOME if above operation failed, -p specifies parent directory
        dir=$HOME/$COIN_NAME-temp
        mkdir -p $dir
    fi
...
apt-get update >/dev/null 2>&1
apt-get install ufw unzip -y >/dev/null 2>&1
